...
class Trainer:
    ...
    def __init__(
        ...
        # Label smoothing  - # issue 1 - happens with label_smoothing_factor
        #self.args.label_smoothing_factor = 0.1
    ...
    def train(
        ...
        **kwargs,
        ...
        train_dataset_is_sized = isinstance(self.train_dataset, collections.abc.Sized) # returns boolean
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        """
        model.train()
        # input ids, attention mask, label , decoder attention mask\
    ...
    def wordmoverdistance(self,word2vecmodel,sentence1,sentence2):
        n = len(sentence1)
        distance = 0 
        for s1,s2 in zip(sentence1,sentence2):
            preprocess_s1 = self.preprocess(s1)
            preprocess_s2 = self.preprocess(s2)

            distance+= word2vecmodel.wmdistance(preprocess_s1, preprocess_s2)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        """
        # changed label_smoother condition not None to None 
        if self.label_smoother is not None and "labels" in inputs:
            labels = inputs.pop("labels") # changed from inputs.pop("labels")
        else:
            labels = None
        ...
        inputs1['decoder_attention_mask'] = inputs['decoder_attention_mask']
        inputs1['key_select'] = 0 

        inputs2 = {}
        inputs2['input_ids'] = inputs['input_ids']
        inputs2['labels'] = inputs['labels1']
        inputs2['attention_mask'] = inputs['attention_mask']
        inputs2['decoder_attention_mask'] = inputs['decoder_attention_mask1']
        inputs2['key_select'] = 1 

        inputs3 = {}
        inputs3['input_ids'] = inputs['input_ids']
        inputs3['labels'] = inputs['labels2']
        inputs3['attention_mask'] = inputs['attention_mask']

        inputs3['decoder_attention_mask'] = inputs['decoder_attention_mask2']
        inputs3['key_select'] = 2 
        ...
        if labels is not None:
            ...
            # loss 2 - mover distance 
            # tokenizer.batchdecode(outputs)
            # changing this line 
            #loss1 = self.label_smoother(outputs, labels) 
            #print("labels : ",labels)
            #loss1=outputs[0] # added from else part 
            #target_sentence = self.tokenizer.batch_decode(labels,skip_special_tokens=True, clean_up_tokenization_spaces=True)
            #print("target sentence : ",target_sentence)
            # https://discuss.huggingface.co/t/t5-forward-pass-versus-generate-latter-outputs-non-sense/4831/3
            #print("output logits : \n",outputs.logits)
            #output_sentence = self.tokenizer.batch_decode(torch.argmax(outputs.logits, dim=2).tolist(), skip_special_tokens=True,clean_up_tokenization_spaces=True)
            #print("output_batch_decode_input :\n",torch.argmax(outputs.logits, dim=2).tolist())
            #print("output_sentence : \n",output_sentence)
            #sys.exit()
            #output_sentence = self.tokenizer.batch_decode(outputs,skip_special_tokens=True, clean_up_tokenization_spaces=True)
            #loss2 = self.wordmoverdistance(self.word2vecmodel,
            #                                        output_sentence, 
            #                                         target_sentence) 
            #logger.info(f"MOVER DISTANCE LOSS={loss2}`.")
            ...
            loss3 = outputs3["loss"] if isinstance(outputs3, dict) else outputs3[0]
            """
            target_sentence = self.tokenizer.batch_decode(inputs["labels"],skip_special_tokens=True, clean_up_tokenization_spaces=True)
            output_sentence = self.tokenizer.batch_decode(torch.argmax(outputs1.logits, dim=2).tolist(), skip_special_tokens=True,clean_up_tokenization_spaces=True)

            target_sentence1 = self.tokenizer.batch_decode(inputs["labels1"],skip_special_tokens=True, clean_up_tokenization_spaces=True)
            output_sentence1 = self.tokenizer.batch_decode(torch.argmax(outputs2.logits, dim=2).tolist(), skip_special_tokens=True,clean_up_tokenization_spaces=True)

            target_sentence2 = self.tokenizer.batch_decode(inputs["labels2"],skip_special_tokens=True, clean_up_tokenization_spaces=True)
            output_sentence2 = self.tokenizer.batch_decode(torch.argmax(outputs3.logits, dim=2).tolist(), skip_special_tokens=True,clean_up_tokenization_spaces=True)

            wmd1 = self.wordmoverdistance(self.word2vecmodel,
                                                      output_sentence, 
                                                      target_sentence)
            wmd2 = self.wordmoverdistance(self.word2vecmodel,
                                                      output_sentence1, 
                                                      target_sentence1)
            wmd3 = self.wordmoverdistance(self.word2vecmodel,
                                                      output_sentence2, 
                                                      target_sentence2)
            loss = (((1-self.loss_multiplier)*(loss1 + loss2 +loss3))+self.loss_multiplier*(wmd1+wmd2+wmd3))/3
            """
    ...
    def prediction_step(
        ...
        has_labels = all(inputs1.get(k) is not None for k in self.label_names)
        ...
        if has_labels:
            labels = nested_detach(tuple(inputs1.get(name) for name in self.label_names))
            if len(labels) == 1:
                labels = labels[0]
        else:
            labels = None
