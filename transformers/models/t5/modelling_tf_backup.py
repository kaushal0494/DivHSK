...
stop_words.extend(['</s>','The','made','.','','I','said','also','des','would','A','u','sh','<pad>','<unk>','nji','kul','mur','...'])
####################################################
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        #print("mask in t5attention : ",mask.shape)
        ...
        scores += position_bias #[:32] is not there before 
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        #print("entered t5 stack")
        # Model parallel
        #print("input ids shape : ",input_ids.shape)
        #print("attentions : ",attention_mask.shape)
        ...
        #print("t5 stack - input ids : ",input_ids)
        #print("t5 stack inp ids shape : ",input_ids.shape)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def subkeytoKeywordConverter(self,input_sentence,g):
        """
        ...
        if len(final_keys)<10:
            final_keys = self.pad_sequence(final_keys,10-len(final_keys))
    ...
    def get_group_attention_sks(self,ind_sks_attentions):
        ...
        g1_attentions = self.GetAttention(g1_keys,ind_sks_attentions[:4])
    ...
    def get_subkey_helper(self,mh_attention, input_id):
        ...
        for i in range(num_heads): 
            ind_sks_attentions[i] = self.get_individual_attention_sks(mh_attention[i],input_id)

        group_sk1_attentions, group_sk2_attentions, group_sk3_attentions = self.get_group_attention_sks(ind_sks_attentions)
    ...
    def SelectionProcedure(self,n,r,g1,g2,g3,g1g2,g2g3,g1g3,g1g2g3,g_attention):
        ...
        no_g1 = self.GroupSelection(g1,g_attention[0],r)
        ...
        if len(g1)<10:
            g1 = self.pad_sequence(g1,10-len(g1))
        if len(g2)<10:
            g2 = self.pad_sequence(g2,10-len(g2))
        if len(g3)<10:
            g3 = self.pad_sequence(g3,10-len(g3))
    ...
    def SelectTopKeywords(self,keywords_group, group_attention,n = 5,r = 2):
        ...
        return self.SelectionProcedure(n,r,onlyg1,onlyg2,onlyg3,g1g2,g2g3,g1g3,int_g1g2g3,group_attention)
    ...
    def get_subkeyword_ids(self,g1,g2,g3,input_sentence = None):
        """
        ...
        g_keys1, g_keys2, g_keys3 = self.SelectTopKeywords([g1[0],g2[0],g3[0]],[g1[1],g2[1],g3[1]])
        ...
        sk_ids1 = self.customtokens_to_ids(g_keys1)
        sk_ids2 = self.customtokens_to_ids(g_keys2)
        sk_ids3 = self.customtokens_to_ids(g_keys3)
        
        if input_sentence is not None:
            keys1 = self.subkeytoKeywordConverter(input_sentence, g_keys1)
            keys2 = self.subkeytoKeywordConverter(input_sentence, g_keys2)
            keys3 = self.subkeytoKeywordConverter(input_sentence, g_keys3)
            
            #print("Keys 1 : \n",g_keys1)
            #print('Keys 2 : \n',g_keys2)
            #print("Keys 3 : \n",g_keys3)
            
            key_ids1 = self.EncodeKeyword(keys1)
    ...
    def get_subkey_representations(self,multihead_attentions,input_ids):
        ...
        for i in range(n):
            ...
            group1_atts, group2_atts, group3_atts = self.get_subkey_helper(mh_attention,input_id)
            subkeys1[i], subkeys2[i], subkeys3[i] = self.get_subkeyword_ids(group1_atts,group2_atts,group3_atts,input_sentence)
    ...
    def forward(
        ...
        added for additional headline labels 
        # added labels1 and labels2 
        # decoder_attention_mask1 & decoder_attention_mask2 

        generate_flag = False # false by default during training, if kept true,
        during testing it generates the keyword subkey ids using keyencoder
        and returns it

        beam_flag = True # false by default during training, if kept true, during
        testing encoder-decoder model is called for generation and headlines id 
        ...
        # PHG Addition 
        # during test phase beam flag is made true and it will go to this if condition
        if beam_flag:
    ...
    # PHG Addition 
        ...
        if generate_flag: # same encoder during training & generation 
            ...
            #subkey_ids = torch.Tensor(subkey_representations)
        ...
        #print("keyencoder outputs  :\n",keyencoder_outputs) #torch.Size([32, 12, 128, 128])
        #print("keyencoder attentions : ",keyencoder_outputs.attentions[0].shape) #torch.Size([32, 12, 128, 128])
        #print(keyencoder_outputs.attentions[0][0].shape) #torch.Size([12, 128, 128])
        #print("input_ids shape : ",input_ids.shape) #torch.Size([32, 128])
        multihead_attentions = keyencoder_outputs.attentions[0]
        #print(multihead_attentions.shape[0])
        #for i in range(multihead_attentions.shape[0]):
        #    print(i)
        #    print(multihead_attentions[i].shape)
        subkey_representations = self.get_subkey_representations(multihead_attentions,input_ids)
        #print("subkey rep : ",subkey_representations)
        # Determine maximum length
        #print("Key encoder outs : ",keyencoder_outputs)
        # need to convert subkeyids list to tensor - facing issue # add <pad> token id 
        #subkey_ids = torch.Tensor(subkey_representations)

        #print("subkey_ids : ",subkey_ids.shape)
        #subkey_input_ids = subkey_representations[0]+[SEP] + subkey_representations[1] + [SEP] + subkey_representations[2] + [SEP]
        """
        attentions = [] # additional attention for subkey ids 
        curr_batch = attention_mask.shape[0]
        for i in range(3):
            curr_attentions =[]
            for j in range(curr_batch):
                curr_attentions.append([1]*10)
            attentions.append(curr_attentions)
        attentions_sks = torch.Tensor(attentions)
        #[3,32,10] - [1,1,1,...]
        # attention mask for subkey ids for given batch 
        """
        total_loss = torch.Tensor(0).to("cuda:0") # initialize total loss
        loss_list = [None]*3 # initialize loss list 

        # loop thrice for each group of subkey ids that we got 
        for i,(l,dec_att_mask) in enumerate(zip([labels,labels1,labels2],[decoder_attention_mask,decoder_attention_mask1,decoder_attention_mask2])):
            # Encode if needed (training, first prediction pass)
            if encoder_outputs is None:
            # modify - 8 
                # Convert encoder inputs in embeddings if needed
                #curr_subkey_id = subkey_ids[i] 

                #curr_attention = attentions_sks[i]
                #print("curr_subkey id :",curr_subkey_id)
                #print("curr_attention : ",curr_attention)
                inp_ids, att_mask = self.GetValidInputs(input_ids, attention_mask, subkey_representations[i])
                #print("curr inp shape : ",torch.Tensor(curr_subkey_id).to('cuda:0').long().shape)
                #print("inp id shape : ",input_ids[...,10:].shape)
                #print("curr att shape : ",torch.Tensor(curr_attention).to('cuda:0').long().shape)
                #print("att shape : ",attention_mask[...,10:].shape)

                # merge curr_subkey_id with input_ids and terminate last 10 
                # same for attention mask too
                #print(torch.Tensor(curr_subkey_id).shape)
                #inp_ids = torch.cat((torch.Tensor(curr_subkey_id).to('cuda:0').long(),input_ids[...,n_sk:]),1)
                #att_mask = torch.cat((torch.Tensor(curr_attention).to('cuda:0').long(),attention_mask[...,n_sk:]),1)
      
                assert inp_ids.shape==input_ids.shape 
                
                encoder_outputs = self.encoder(
                    input_ids= inp_ids, #strip last 6 
            ...
            if l is not None and decoder_input_ids is None and decoder_inputs_embeds is None:
                # get decoder inputs from shifting lm labels to the right
                decoder_input_ids = self._shift_right(l)

            # If decoding with past key value states, only the last tokens
            # should be given as an input
            if past_key_values is not None:
                assert l is None, "Decoder should not use cached key value states when training."
                if decoder_input_ids is not None:
                    decoder_input_ids = decoder_input_ids[:, -1:]
                if decoder_inputs_embeds is not None:
                    decoder_inputs_embeds = decoder_inputs_embeds[:, -1:]
            ...
            decoder_outputs = self.decoder(
                input_ids=decoder_input_ids,
                attention_mask=dec_att_mask, # changed 
                inputs_embeds=decoder_inputs_embeds,
                past_key_values=past_key_values,
                encoder_hidden_states=hidden_states,
                encoder_attention_mask=attention_mask,
            ...
            if labels is not None:
                ...
                # pass 3 labels 
                loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), l.view(-1))
                loss_list[i] = loss # added loss to loss lit 
                # TODO(thom): Add z_loss https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/layers.py#L666

        total_loss = (loss_list[0]+loss_list[1]+loss_list[2])/3
        ...
        return Seq2SeqLMOutput(
            loss=total_loss, # modify loss to total loss 
            logits=lm_logits,
            past_key_values=decoder_outputs.past_key_values,
            decoder_hidden_states=decoder_outputs.hidden_states,
            decoder_attentions=decoder_outputs.attentions,
