...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def get_subkey_representations(self,multihead_attentions,input_ids):
        ...
        k = multihead_attentions.shape[0]
        subkeys1 = [None]*k
        subkeys2 = [None]*k
        subkeys3 = [None]*k
        ...
        for i in range(k):
            ...
            subkeys1[i], subkeys2[i], subkeys3[i] = self.get_subkeyword_ids(group1_atts,group2_atts,group3_atts,input_sentence) #input sentence for keyword extension 3
    ...
    # 2 - 1
    def get_individual_attention_sks(self,attention,input_id):
        """
        ...
        vk, ik = attention_wts.topk(self.top_k)
        attention_store = {}

        for i in ik.reshape(1,self.top_k):
    ...
    def get_group_attention_sks(self,ind_sks_attentions):
        ...
        for i,j,k in zip(range(4),range(4,8),range(8,12)):
    ...
    def get_subkeyword_ids(self,g1,g2,g3,input_sentence = None):
        ...
        g_keys1, g_keys2, g_keys3 = self.SelectTopKeywords([g1[0],g2[0],g3[0]],[g1[1],g2[1],g3[1]]) # 3 - 1
        ...
        if input_sentence is not None:
            keys1 = self.subkeytoKeywordConverter(input_sentence, g_keys1) # 3 - 3
    ...
    # 3 - 1
    def SelectTopKeywords(self,keywords_group, group_attention):
        """
        ...
        return self.SelectionProcedure(onlyg1,onlyg2,onlyg3,g1g2,g2g3,g1g3,int_g1g2g3,group_attention) # 3 - 1 - 1 

    # 3 - 1 - 1 
    def SelectionProcedure(self,g1,g2,g3,g1g2,g2g3,g1g3,g1g2g3,g_attention):
        """
        ...
        no_g1 = self.GroupSelection(g1,g_attention[0],self.r) # 3- 1 - 1 - 1
        o_g1 = self.GroupSelection([g1g2,g1g3,g1g2g3],g_attention,self.n-len(no_g1),1,'o')

        no_g2 = self.GroupSelection(g2,g_attention[1],self.r)
        o_g2 = self.GroupSelection([g2g3,g1g2,g1g2g3],g_attention,self.n-len(no_g2),2,'o')

        no_g3 = self.GroupSelection(g3,g_attention[2],self.r)
        o_g3 = self.GroupSelection([g1g3,g2g3,g1g2g3],g_attention,self.n-len(no_g3),3,'o')
        ...
        g1 = list(chain(*[group1[i:i+o] + [ele] if len(group1[i:i+o]) == o else group1[i:i+o] for i in range(0, len(group1), o)]))
        g2 = list(chain(*[group2[i:i+o] + [ele] if len(group2[i:i+o]) == o else group2[i:i+o] for i in range(0, len(group2), o)]))
        g3 = list(chain(*[group3[i:i+o] + [ele] if len(group3[i:i+o]) == o else group3[i:i+o] for i in range(0, len(group3), o)]))
       
        if len(g1)<(2*self.n):
            g1 = self.pad_sequence(g1,(2*self.n)-len(g1)) # 3- 1 - 1 - 2
        if len(g2)<(2*self.n):
            g2 = self.pad_sequence(g2,(2*self.n)-len(g2))
        if len(g3)<(2*self.n):
            g3 = self.pad_sequence(g3,(2*self.n)-len(g3))
    ...
    def subkeytoKeywordConverter(self,input_sentence,g):
        ...
        if self.keyword_extension:
            keywords = self.WordNetKeywords(keywords)
        ...
        if len(final_keys)<(2*self.n):
            final_keys = self.pad_sequence(final_keys,(2*self.n)-len(final_keys))

        return final_keys
    
    def WordNetKeywords(self,keywords,select=1):
        final_keywords = set()
        for words in keywords:
            key = self.RemovePunctuation(words)
            synonyms = set()
            for syn in wordnet.synsets(key):
                for lm in syn.lemmas():
                    if len(lm.name())>2:
                        synonyms.add(lm.name())
            final_keywords.add(key)
            s_list = list(synonyms)
            for i in range(min(select,len(s_list))):
                final_keywords.add(s_list[i])
    ...
    def forward(
        ...
        if not beam_flag:
            ...
            multihead_attentions = keyencoder_outputs.attentions[0]
            #print(multihead_attentions[0])
            subkey_representations = self.get_subkey_representations(multihead_attentions,input_ids)
            input_ids, attention_mask = self.GetValidInputs(input_ids, attention_mask, subkey_representations[key_select])
        # Encode if needed (training, first prediction pass)
        if encoder_outputs is None:
