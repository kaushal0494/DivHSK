...
class GenerationMixin:
    ...
    def GetValidInputs(self,input_ids, attention_mask, subkey_representations):
        ...
        #print("subkey rep in getvalid inp : ",subkey_representations)
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        #attentions = []
        attention_masks = model_kwargs['attention_mask']
        #curr_batch = attention_masks.shape[0]
        #for i in range(3):
        #    curr_attentions =[]
        ...
        topkp_key_out = []
        for i in range(len(keyword_out)):
            #curr_subkey_id = keyword_out[i]
            #curr_attention = attentions_sks[i]
            #print("main input ids : ",main_input_ids)
            #print("keyword out : ",keyword_out[i])
            #print("input ids bef : ",main_input_ids)
            input_ids, model_kwargs['attention_mask'] = self.GetValidInputs(main_input_ids,attention_masks, keyword_out[i])
            #print("input_ids_after : ",input_ids)
            #sys.exit()
            curr_input_ids = input_ids 
            #print("input ids in loop : ",input_ids)
            #print("curr subkey id : ",curr_subkey_id)
            #print("curr atten : ",curr_attention)
            #input_ids = torch.cat((torch.Tensor(curr_subkey_id).to('cuda:0').long(),main_input_ids[...,10:]),1) 
            #print("IDS : ",input_ids)
            #model_kwargs['attention_mask'] = torch.cat((torch.Tensor(curr_attention).to('cuda:0').long(),attention_masks[...,10:]),1)
        ...
        # Storing encoder_input_ids for logits_processor that could use them
            ...
            if is_greedy_gen_mode:
                ...
                # sample
                topkp_key_out.append(self.sample(
                    input_ids,
                    logits_processor=logits_processor,
                ...
                input_ids1, model_kwargs = self._expand_inputs_for_generation(
                    curr_input_ids, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder, **model_kwargs
                )
                
                model_kwargs['input_ids1'] = input_ids1
                #print("model kwargs : \n",model_kwargs)
        ...
        return topkp_key_out # list of 3 tokens for decode 
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        """
        # init values
        #print("sample func - input ids : ",input_ids)
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        if max_length is not None:
            warnings.warn(
        ...
        while True:
            ...
            # forward pass to get next token
            outputs = self(
                **model_inputs, # changed from **model_inputs
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
                beam_flag = True, 
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            model_inputs['input_ids'] = model_kwargs['input_ids1']
            model_inputs.pop('encoder_outputs')
            outputs = self(
                **model_inputs,
                return_dict=True,
                output_attentions=output_attentions,
