...
class Trainer:
    ...
    def getDistance(self,sentence1,sentence2):
        d = self.similarity_model.wmdistance(self.preprocess(sentence1), self.preprocess(sentence2))
        return d
        
# reference : https://towardsdatascience.com/bert-for-measuring-text-similarity-eec91c6bf9e1
    def WMD(self,sentence1,sentence2):
        n = len(sentence1)
        distance = list(map(self.getDistance,sentence1,sentence2))
        #for s1,s2 in zip(sentence1,sentence2):
        #    curr_sentence = [s1,s2]
        #    distance+= self.getDistance(curr_sentence)
        print("wmd : ",np.sum(distance)/n)
        return np.round(np.sum(distance)/n,4)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if labels is not None:
            ...
            target_sentence = self.tokenizer.batch_decode(inputs.pop("labels"),skip_special_tokens=True, clean_up_tokenization_spaces=True)
            output_sentence = self.tokenizer.batch_decode(torch.argmax(outputs.logits, dim=2).tolist(), skip_special_tokens=True,clean_up_tokenization_spaces=True)
            
            loss2 = self.WMD(target_sentence,output_sentence)
